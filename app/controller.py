...
async def read_a111(file: BytesIO):
    ...
    tipo_top_p = "Unknown TIPO top_p Temperature"  # Initialize TIPO Top_P Temperature by default
    tipo_top_k = "Unknown TIPO top_k Temperature"  # Initialize TIPO Top_k Temperature by default
    ...
    try:
        ...
        with Image.open(file) as img:
            ...
            if prompt_end != -1:
                ...
                prompt = "❌ Prompt was not found or an error occurred when receiving it."

            # Extraction of Negative Prompt (to Steps)
            negative_start = parameter.find("Negative prompt:")
            if negative_start != -1 and steps_start != -1:
                negative_prompt = parameter[negative_start + len("Negative prompt:"):steps_start].strip()
            else:
            ...
            model = next((p.split(": ")[1] for p in parameter.split(", ") if p.startswith("Model:")), "Unknown Model")
            sampler = next((p.split(": ")[1] for p in parameter.split(", ") if p.startswith("Sampler:")), "Unknown Sampler")
            cfg_scale = next((p.split(": ")[1] for p in parameter.split(", ") if p.startswith("CFG scale:")), "Unknown CFG Scale")
            schedule = next((p.split(": ")[1] for p in parameter.split(", ") if p.startswith("Schedule type:")), "Unknown Schedule type")

            # Extracting TIPO parameters manually through searching for curly brackets
            tipo_start = parameter.find("TIPO Parameters: \"{")
            tipo_data = None
            if tipo_start != -1:
                tipo_end = parameter.find("}\"", tipo_start)  # Find the end of the line with parameters
                if tipo_end != -1:
                    tipo_data = parameter[tipo_start + len("TIPO Parameters: \""):tipo_end + 1].strip()
                    ...
                    tipo_nlprompt_match = re.search(r'TIPO nl prompt: ([^,]+)', parameter)
            ...
            if sampler:
                message.append(f"**📦 Sampler:** `{sampler}`  **Schedule:** `{schedule}`")
            if cfg_scale:
            ...
            if tipo_format != "Unknown TIPO Format": # Display Tipo Format only if it is found
                message.append(f"> **✏️ Format:** `{tipo_format}`")
                message.append(f"> **✏️ Ban Tags:** `{tipo_ban}`")
                message.append(f"> **✏️ Temperature: `{tipo_temp}`  top_p: `{tipo_top_p}`  top_k: `{tipo_top_k}`**")
            else:
...
async def read_novelai(file: BytesIO, result=None):
    ...
    try:
        ...
        if metadata.Comment:
            ...
            sm_dyn = str(metadata.Comment.sm_dyn) if metadata.Comment.sm_dyn is not None else ""
            scale = str(metadata.Comment.scale) if metadata.Comment.scale is not None else ""
            cfg_rescale = str(metadata.Comment.cfg_rescale) if metadata.Comment.cfg_rescale is not None else ""
            rq_type = metadata.Comment.request_type if metadata.Comment.request_type else ""
            ...
            if rq_type == "PromptGenerateRequest":
                ...
                mode = "Inpainting"
            if metadata.Comment.reference_strength_multiple or metadata.Comment.reference_information_extracted_multiple:
                mode += "+VibeTransfer"
        ...
        if scale or cfg_rescale:
            message.append(f"**📦 Guidance Scale:** `{scale}`  Rescale: `{cfg_rescale}`")
        if read_source:
        ...
        if mode:
            ...
            message.append("❌ An error occurred while processing the request. " + metadata.get('error', ''))
...
class BotRunner(object):
    ...
    async def tagger(self, file, hidden_long_text=False) -> str:
        ...
        if read_message and hidden_long_text:
            ...
            infer_message = ["**💮 Your Guessed Prompt:**", f"```{result.anime_tags}```"]
        elif read_message:
            infer_message = ["**💮 Your Guessed Prompt:**", f"```{result.anime_tags}```"]
        else:
            infer_message = []
    ...
    async def run(self):
        ...
        @bot.message_handler(commands="tag", chat_types=["supergroup", "group", "private"])
        async def tag(message: types.Message):
            ...
            if reply_message_doc:
                ...
                return await self.bot.reply_to(message, text="🍡 Please reply to an document/image with this command")
